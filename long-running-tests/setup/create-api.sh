...
create_api() {
    ...
    do
        ...
        local revisionUuid=$($curl_command -H "Authorization: Bearer $create_access_token" -H "Content-Type: application/json" -X POST -d "$(revision_deploy_request $vhost)" ${base_https_url}/api/am/publisher/v4/apis/${api_id}/deploy-revision?revisionId=${rev_id} | jq -r '.[0] | .revisionUuid')
        ...
        if [ ! -z "$mediation_sequence" ]; then
            ...
            until [ $n -ge 50 ]; do
                ...
                if [ ! -z $updated_api_id ] && [ ! $updated_api_id = "null" ]; then
                    ...
                    local rev_id_2=$($curl_command -H "Authorization: Bearer $admin_token" -H "Content-Type: application/json" -X POST -d '{"description": "second revision"}' ${base_https_url}/api/am/publisher/v4/apis/${updated_api_id}/revisions | jq -r '.id')
                    local revisionUuid=$($curl_command -H "Authorization: Bearer $admin_token" -H "Content-Type: application/json" -X POST -d "$(revision_deploy_request $vhost)" ${base_https_url}/api/am/publisher/v4/apis/${updated_api_id}/deploy-revision?revisionId=${rev_id_2} | jq -r '.[0] | .revisionUuid')
                    sleep 3
                    break
                fi
                n=$(($n + 1))
        ...
        do
            ...
            local application_id=$($curl_command -H "Authorization: Bearer $app_access_token" "${base_https_url}/api/am/devportal/v3/applications?query=$app_name" | jq -r '.list[0] | .applicationId')
            
            echo "Subscribing $api_name API to $app_name"
            local subscription_id=$($curl_command -H "Authorization: Bearer $sub_manage_token" -H "Content-Type: application/json" -d "$(subscription_request $api_id $application_id)" "${base_https_url}/api/am/devportal/v3/subscriptions" | jq -r '.subscriptionId')
            if [ ! -z $subscription_id ] && [ ! $subscription_id = "null" ]; then
